...
4. To link the real PJSIP SDK, either point the Podfile at a vendor podspec (`USE_PJSIP_POD=1 PJSIP_PODSPEC=https://example.com/PJSIP.podspec pod install --repo-update`) or add prebuilt frameworks, expose `pjsua` headers in a bridging header, and define `USE_PJSIP` in **Other Swift Flags** so the native wrapper is compiled.
